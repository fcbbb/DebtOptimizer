...
function displayMessages(messages) {
    ...
    console.log('=== 显示消息前 ===');
    console.log('chatMessages.scrollHeight (显示前):', chatMessages.scrollHeight);
    console.log('chatMessages.clientHeight (显示前):', chatMessages.clientHeight);
    console.log('chatMessages.offsetHeight (显示前):', chatMessages.offsetHeight);
    ...
    console.log('chatMessages.offsetHeight (清空后):', chatMessages.offsetHeight);
    ...
    console.log('=== 添加所有消息后 ===');
    console.log('chatMessages.scrollHeight (添加所有后):', chatMessages.scrollHeight);
    console.log('chatMessages.clientHeight (添加所有后):', chatMessages.clientHeight);
    console.log('chatMessages.offsetHeight (添加所有后):', chatMessages.offsetHeight);
...
function addUserMessage(content, timestamp = Date.now(), save = true) {
    ...
    console.log('=== 添加用户消息前 ===');
    console.log('chatMessages.scrollHeight (添加前):', chatMessages.scrollHeight);
    console.log('chatMessages.clientHeight (添加前):', chatMessages.clientHeight);
    console.log('chatMessages.offsetHeight (添加前):', chatMessages.offsetHeight);
    ...
    console.log('=== 添加用户消息后 ===');
    console.log('新消息元素offsetHeight:', messageElement.offsetHeight);
    console.log('chatMessages.scrollHeight (添加后):', chatMessages.scrollHeight);
    console.log('chatMessages.clientHeight (添加后):', chatMessages.clientHeight);
    console.log('chatMessages.offsetHeight (添加后):', chatMessages.offsetHeight);
...
function addAIMessage(content, timestamp = Date.now(), save = true) {
    ...
    console.log('chatMessages.clientHeight (添加后):', chatMessages.clientHeight);
    console.log('chatMessages.offsetHeight (添加后):', chatMessages.offsetHeight);
    console.log('新添加的消息内容:', messageElement.innerHTML);
...
function handleWebSocketMessage(data) {
    switch (data.type) {
        ...
        case 'error':
            // 错误处理
            hideTypingIndicator();
            addAIMessage('抱歉，处理您的请求时出现了错误。请稍后重试。');
...
function addImageMessageToChat(imageId) {
    ...
    messageElement.innerHTML = `
        ...
        <div class="message bot-message">
            <p>我已经收到了您上传的图片:</p>
            <img src="/media/temp/${imageId}" alt="处理后的图片" style="max-width: 300px; max-height: 300px; border-radius: 8px;">
...
function finalizeCurrentMessages() {
    ...
    if (chatMessages !== document.getElementById('chat-messages')) {
        console.error('❌ chatMessages 引用已失效！');
    }
    console.log('收到完整消息:', currentAIResponse);
    hideTypingIndicator();

    // ✅ 只选择可见的 AI 消息容器
    const aiMessages = Array.from(chatMessages.querySelectorAll('.message-container.bot')).filter(el => window.getComputedStyle(el).display !== 'none');

    console.log('可见的 AI 消息数量:', aiMessages.length);
    if (aiMessages.length > 0) {
        const lastAIMessage = aiMessages[aiMessages.length - 1];
        const messageDiv = lastAIMessage.querySelector('.bot-message');

        console.log('操作的 AI 容器:', lastAIMessage);
        console.log('当前内容:', messageDiv ? messageDiv.textContent : 'null');
...
function handleImageUpload(event) {
    ...
    .then(data => {
        if (data.image_id) {
            // 图片上传成功，发送包含图片ID的消息
            sendImageMessage(data.image_id);
        } else {
...
// 新增：发送图片消息到后端的函数
function sendImageMessage(imageId) {
    if (!imageId) return;
    
    // 添加用户图片消息到聊天界面
    addUserImageMessage(imageId);
...
function addUserImageMessage(imageId) {
    ...
    messageElement.innerHTML = `
        <div class="message user-message">
            <img src="/media/temp/${imageId}" alt="上传的图片" style="max-width: 300px; max-height: 300px; border-radius: 8px;">
            <div class="message-time">${formatTime(Date.now())}</div>
        </div>
        <div class="avatar user-avatar">我</div>
    `;
